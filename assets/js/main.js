(function () {
  ...
  var PLACEHOLDER_IMG = "data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==";
  ...
  // ---------- INIT ----------
  function primeDataImages() {
    qsa("img[data-image-key]").forEach(function (img) {
  ...
  function hydrateDataImages() {
    if (!window.BOOK_HAVEN_IMAGES) return;
    qsa("img[data-image-key]").forEach(function (img) {
      var key = img.getAttribute("data-image-key");
      var src = window.BOOK_HAVEN_IMAGES[key];
      if (src) {
